      ...
      `).join(""):'<div class="empty">No saved views yet. Save a filter + browser combination for faster operator workflows.</div>')}function pt(e,t,n){let i=[...e];return t===`${n}_asc`?(i.sort((r,o)=>String(r[n]||"").localeCompare(String(o[n]||""))),i):t===`${n}_desc`?(i.sort((r,o)=>String(o[n]||"").localeCompare(String(r[n]||""))),i):t==="grain_asc"?(i.sort((r,o)=>String(r.grain_key||"").localeCompare(String(o.grain_key||""))),i):t==="grain_desc"?(i.sort((r,o)=>String(o.grain_key||"").localeCompare(String(r.grain_key||""))),i):(t==="kpi_asc"&&i.sort((r,o)=>String(r.kpi_field||r.kpi_json&&Object.keys(r.kpi_json)[0]||"").localeCompare(String(o.kpi_field||o.kpi_json&&Object.keys(o.kpi_json)[0]||""))),i)}function Ct(){let e=document.querySelectorAll("[data-api-tab]"),t=document.querySelector(".setup-wizard");if(document.querySelectorAll("[data-wizard-step]").forEach(i=>{let r=i.dataset.wizardStep;i.classList.toggle("active",r===s.currentApiTab)}),!e.length){document.querySelectorAll("[data-api-section]").forEach(i=>i.classList.add("active"));return}document.querySelectorAll("[data-api-section]").forEach(i=>{let r=i.dataset.apiSection,o=r===s.currentApiTab||s.currentApiTab==="summary"&&(r==="summary"||r==="incidents"||r==="history");i.classList.toggle("active",o)}),e.forEach(i=>{i.classList.toggle("active",i.dataset.apiTab===s.currentApiTab)});let n=document.getElementById("magic-baseline-button");n&&n.classList.toggle("brand-glow",s.currentApiTab==="uploads")}function Tt(e){s.selectedIncident=e,a.drawerTitle.textContent=`${e.endpoint_path} \u2022 ${e.kpi_field}`;let t=e.timeline||[],n=Number(e.pct_change||0),i=Math.abs(n),r=n>0,o=Number(e.confidence),l=Number.isFinite(o)?Math.round(o*100):null,d=l==null?"Confidence is not available yet (still learning this pattern).":`Confidence: ${l}%.`,c={active:"Needs review",acknowledged:"In review",snoozed:"Snoozed",suppressed:"Suppressed",resolved:"Resolved"},p={critical:"High",high:"High",medium:"Medium",low:"Low"},_=e.detected_at?ne(e.detected_at):"Not recorded yet",A=n===0?"equal":r?"higher":"lower",N=String(e.endpoint_path).replace(/'/g,"\\\\'"),H=i>=40?`<strong>Major ${r?"increase":"drop"}</strong> compared to the target.`:`<strong>${r?"Higher":"Lower"}</strong> than expected.`;a.drawerBody.innerHTML=`
        <div class="row-card drawer-quick-summary">
          ...
          <ul class="drawer-bullets">
            <li><strong>${e.kpi_field}</strong> is ${u(i)}% ${A} than the expected value.</li>
        ...
        <div class="meta-grid">
          ...
          <div class="meta-card"><strong>Change details</strong><span>${e.change_since_last_healthy_run||`${e.kpi_field} moved by ${u(n)}% from target.`}</span></div>
        </div>
        ...
        <div class="suggestion-toolbar">
          ...
          <button class="action secondary" style="width:100%; justify-content:center;" type="button" onclick="quickFixBaseline(${e.id})">
            Accept ${u(e.actual_value)} as new target
          </button>
...
<body style="margin:0;background:#f4efe7;color:#1f1912;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
  ...
  `}function xs(e){let t={healthy:0,warning:0,anomaly:0,unconfirmed:0};e.forEach(o=>{let l=t[o.status]!==void 0?o.status:"warning";t[l]+=1});let n=Math.max(1,e.length),i=0;return`<svg class="chart-svg chart-svg-compact" viewBox="0 0 100 56" preserveAspectRatio="none" aria-hidden="true">${[["healthy","var(--healthy)"],["warning","var(--warning)"],["anomaly","var(--anomaly)"],["unconfirmed","var(--ink-muted)"]].map(([o,l])=>{let d=t[o]/n*100,c=`<rect x="${i}" y="18" width="${d}" height="18" rx="4" fill="${l}"></rect>`;return i+=d,c}).join("")}</svg>`}function As(e){let t={};(e.recent_history||[]).forEach(p=>{Object.entries(p.kpi_json||{}).forEach(([_,A])=>{Number.isFinite(A)&&(t[_]=t[_]||[],t[_].push(Number(A)))})});let n=["var(--accent)","var(--healthy)","var(--warning)","var(--anomaly)"],i=Object.entries(t).slice(0,4);if(!i.length)return"";let r=i.flatMap(([,p])=>p),o=Math.min(...r),d=Math.max(...r)-o||1;return`<svg class="chart-svg" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">${i.map(([,p],_)=>{let A=p.map((N,H)=>{let j=p.length===1?0:H/(p.length-1)*100,k=100-((N-o)/d*78+10);return`${j},${k}`}).join(" ");return`<polyline fill="none" stroke="${n[_%n.length]}" stroke-width="2.5" points="${A}" />`}).join("")}</svg>`}var js=["method","status","setup","issues"],Ei={method:92,status:92,setup:112,issues:88},Ii=120,Ri=28,Ni=8;function Bs(e){return e.startsWith("scheduler")?"Scheduler":e.startsWith("router.upload")||e.startsWith("router.save_references")?"Upload":e.startsWith("router.save_config")||e.startsWith("config.")?"Configuration":e.startsWith("router.")||e.startsWith("middleware.")?"Runtime":"General"}function Bn(e){return e.startsWith("scheduler")||e.startsWith("middleware.process_response")?"high":e.startsWith("router.status")||e.startsWith("router.endpoint_detail")?"medium":"low"}function sn(e){return e.status||"open"}function S(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function Hi(e){return`
  ...
  `}function Gi(e){let t=Ui(e),n=In(),i=Vi(n),r=Is(),o=Xt(r),l=t.length>120?80:50,d=ct(t,s.apiBrowserPage||1,l),c=t.length>Ii?_t(t.length,s.apiBrowserScrollTop||0,{rowHeight:o.rowHeight,windowRows:Ri,overscan:Ni}):null,p=!!c;s.apiBrowserVirtualWindowStart=c?.start||0,s.apiBrowserVirtualWindowEnd=c?.end||0;let _=p?t.slice(c.start,c.end):d.items,A=p?1:d.page,N=p?1:d.totalPages,H=p?`Virtualized \u2022 Showing ${u((c?.start||0)+1)}-${u(Math.min(t.length,c?.end||0))} of ${u(t.length)}`:N>1?`Page ${A} of ${N}`:`${u(t.length)} APIs`,j=p?Math.max(0,(c?.start||0)*o.rowHeight):0,k=p?Math.max(0,(t.length-(c?.end||0))*o.rowHeight):0,P=_.length?_.map(C=>qi(C,n)).join(""):'<div class="empty empty-center api-browser-empty">No APIs match this search.</div>',v=n.map(C=>C==="method"?Et("Method","method",!0):C==="status"?Et("Status","status",!0):C==="setup"?Et("Setup","setup",!0):Et("Issues","issues",!0)).join("");return`
    <div class="api-browser-table-shell" data-api-browser-density="${S(r)}" style="--api-browser-grid-template: ${S(i)}; --api-browser-row-height: ${o.rowHeight}px; --api-browser-table-gap: ${o.tableGap}px; --api-browser-grid-gap: ${o.gridGap}px; --api-browser-head-pad-y: ${o.headPadY}px; --api-browser-head-pad-x: ${o.headPadX}px; --api-browser-row-pad-y: ${o.rowPadY}px; --api-browser-row-pad-x: ${o.rowPadX}px;">
      <div class="api-browser-table-bar">
        ...
        <div class="toolbar compact">${p?"":dt("api-browser",A,N)}</div>
      </div>
      <div class="api-browser-table ${p?"virtualized":""}" role="table" aria-label="API index table">
        ...
        <div class="api-browser-table-body ${p?"virtualized":""}">
          ${p?`<div class="api-browser-spacer" aria-hidden="true" style="height:${j}px"></div>`:""}
          ${P}
  ...
  `}function tr(e,t){let n=Object.entries(e||{}).filter(([,i])=>i!==null&&i!=="");return n.length?n.map(([i,r])=>`${i}=${r}`).join(" \u2022 "):t||"Global grain"}function nr(e){let t=e.request||{},n=[],i=Object.entries(t.path||{}),r=Object.entries(t.query||{}),o=t.body;return i.length&&n.push(`Path ${i.map(([l,d])=>`${l}=${d}`).join(", ")}`),r.length&&n.push(`Query ${r.map(([l,d])=>`${l}=${d}`).join(", ")}`),o&&(Array.isArray(o)&&o.length||!Array.isArray(o)&&Object.keys(o).length)&&n.push(`Body ${JSON.stringify(o)}`),n.join(" \u2022 ")}function It(e){return e==="match"?"healthy":e==="missing_reference"?"acknowledged":e==="missing_kpi"?"warning":e==="mismatch"?"danger":"critical"}function ht(e){return e==="match"?"Matched":e==="mismatch"?"Mismatch":e==="missing_reference"?"No Targets":e==="missing_kpi"?"Missing KPI":e==="error"?"Error":e}function sr(e){return e==="matched"?"Matched":e==="mismatch"?"Mismatch":e==="error"?"Error":e==="skipped"?"Skipped":e||"Unknown"}function ir(e){return e==="matched"?"healthy":e==="mismatch"?"danger":e==="error"?"critical":"acknowledged"}function Xe(e){return String(e||"").replace(/^data\[\]\./,"")}function Ke(e){let t=String(e||"");if(!t)return"";if(!t.includes("|"))return t;let[n,...i]=t.split("|"),r=i.map(o=>o.trim()).filter(Boolean).filter(o=>{let l=o.split("=")[0];return l!=="api_version"&&l!=="label"&&l!=="timestamp"&&l!=="_jin_id"}).sort();return r.length?`${n}|${r.join("|")}`:n}function Rt(e){let t=String(e||"").trim().toLowerCase();return t?t.replace(/\[\]/g,"").replace(/^data\./,"").replace(/^payload\./,""):""}function ft(e){let t=Xe(e).replace(/\[\]/g," ").replace(/[._]/g," ").trim();return t?t.split(/\s+/).map(n=>n.charAt(0).toUpperCase()+n.slice(1)).join(" "):"Metric"}function Ns(e){let t=String(e||"");if(!t.includes("|"))return t||"Global";let[,...n]=t.split("|"),i={};n.forEach(l=>{let d=l.indexOf("=");if(d<=0)return;let c=l.slice(0,d),p=l.slice(d+1);i[c]=p});let r=[i.retailer?`Retailer: ${i.retailer}`:null,i["data[].date"]||i.date?`Date: ${i["data[].date"]||i.date}`:null,i["data[].label"]||i.label?`Label: ${i["data[].label"]||i.label}`:null].filter(Boolean);if(r.length)return r.join(" \u2022 ");let o=Object.entries(i).filter(([l])=>!["api_version","timestamp","_jin_id"].includes(l)).slice(0,3).map(([l,d])=>`${l}=${d}`);return o.length?o.join(" \u2022 "):t}function rr(e){let t=Number(e);return!Number.isFinite(t)||t<0?"\u2014":t<1e3?`${Math.round(t)} ms`:`${(t/1e3).toFixed(1)} s`}function ar(e,t){let n=String(e||"").toLowerCase();if(n==="manual")return"Manual check";if(n==="scheduler")return"Scheduled check";let i=String(t||"").toLowerCase();return i==="manual"?"Manual check":i==="watch"?"Scheduled check":n||i||"Unknown"}function or(e,t=0){let n=String(e||"").toLowerCase(),i=Number(t||0);return(n==="success"||n==="degraded")&&i>0?{pillClass:"danger",label:"Needs attention",tooltip:"Run completed but active mismatches are still open."}:n==="success"?{pillClass:"healthy",label:"Passed",tooltip:"Check finished successfully."}:n==="running"?{pillClass:"acknowledged",label:"Running",tooltip:"Check is currently running."}:n==="error"||n==="failed"?{pillClass:"danger",label:"Failed",tooltip:"Check failed. Open Errors for details."}:n==="skipped"?{pillClass:"warning",label:"Skipped",tooltip:"Check was skipped and did not run."}:{pillClass:"acknowledged",label:n||"Unknown",tooltip:"Run completed with a non-standard status."}}function lr(e){let t=e.dimensions||{},n=t.retailer||t.grain_retailer,i=t["data[].date"]||t.date||t.period,r=t["data[].label"]||t.label,o=[n?`Retailer: ${n}`:null,i?`Date: ${i}`:null,r?`Label: ${r}`:null].filter(Boolean);return o.length?o.join(" | "):tr(e.dimensions,e.grain_key)}function cr(e){return e.status==="match"?"Everything in this uploaded row is within the allowed tolerance.":e.status==="error"?"Jin could not run this row because the API call failed.":`Some values are outside the allowed range${e.tolerance_pct==null?"":` (allowed +/-${Number(e.tolerance_pct).toFixed(1)}%)`}.`}function dr(e){return Number(e.failed_runs||0)>0?"Block release":Number(e.mismatch_runs||0)>0?"Needs attention":"Safe for now"}function Ps(e){let t=Xe(e.kpi_field),n=e.pct_change,i=n==null?null:Number(n);if(e.status==="match")return i==null?`${t} is within the allowed range.`:`${t} is within the allowed range (${Math.abs(i).toFixed(1)}% change).`;if(e.status==="missing_reference")return`No targets are uploaded for ${t}.`;if(e.status==="missing_kpi")return`${t} was not returned by the API for this grain.`;if(e.status==="error")return e.message||`Could not evaluate ${t}.`;if(i==null)return`${t} is outside the allowed range.`;let r=i>0?"higher":"lower";return`${t} is ${Math.abs(i).toFixed(1)}% ${r} than target (outside tolerance).`}function Hs(e){let t=Number(e);if(!Number.isFinite(t))return"\u2014";let n=t>0?"+":"",i=Math.abs(t);return i<1e3?`${n}${i.toFixed(1)}%`:i<1e6?`${n}${Math.round(i).toLocaleString()}%`:`${n}${i.toExponential(2)}%`}function ur(e,t){let n=e.verdict==="matched"?{title:"Upload Analysis Passed",subtitle:e.summary_message,className:"success",icon:"\u2705"}:e.verdict==="mismatch"?{title:"Upload Analysis Found Mismatches",subtitle:e.summary_message,className:"danger",icon:"\u26A0\uFE0F"}:{title:"Upload Analysis Hit Errors",subtitle:e.summary_message,className:"danger",icon:"\u{1F6D1}"},i=m=>{let I=K=>K==="mismatch"?0:K==="error"?1:K==="missing_kpi"?2:K==="missing_reference"?3:4,F=[...m.comparisons||[]].sort((K,se)=>{let be=I(String(K.status||""))-I(String(se.status||""));return be!==0?be:String(K.kpi_field||"").localeCompare(String(se.kpi_field||""))}),L=F.filter(K=>K.status!=="match"),O=F.filter(K=>K.status==="match"),U=nr(m),D=L.slice(0,2).map(K=>{let se=K.pct_change==null?"":` (${Math.abs(Number(K.pct_change)).toFixed(1)}%)`;return`${Xe(K.kpi_field)}: ${ht(K.status)}${se}`}).join(" \u2022 "),le=Math.max(0,L.length-2),Y=L.length?L:O.slice(0,3),pe=L.length?O:O.slice(3),Ae=K=>K.map(se=>`
      ...
      <div class="upload-analysis-card upload-analysis-card-${S(m.status)}">
        ...
        <div class="upload-analysis-kpi-stats tiny">
          ${u(F.length)} KPI(s) \u2022 ${u(L.length)} need attention \u2022 ${u(O.length)} within range
        </div>
    ...
    `},r=e.runs.filter(m=>m.status!=="match"),o=e.runs.filter(m=>m.status==="match"),l=r.slice(0,8),d=r.slice(8),c=r.flatMap(m=>(m.comparisons||[]).filter(I=>I.status!=="match")),p=c.filter(m=>{if(m.status==="error"||m.status==="missing_kpi")return!0;let I=Number(m.pct_change);return Number.isFinite(I)&&Math.abs(I)>=30}),_=c.reduce((m,I)=>{let F=Number(I.pct_change);return Number.isFinite(F)?Math.max(m,Math.abs(F)):m},0),A=new Map;c.forEach(m=>{let I=Xe(m.kpi_field);if(!I)return;let F=Number(m.pct_change),L=Number.isFinite(F)?Math.abs(F):0,O=A.get(I)??0;L>O&&A.set(I,L)});let N=[...A.entries()].sort((m,I)=>I[1]-m[1]).slice(0,3).map(([m,I])=>`${m}${I>0?` (${I.toFixed(1)}%)`:""}`),H=(()=>{let m=Math.max(1,Number(e.requested_grains||0)),I=Number(e.mismatch_runs||0)/m;return Number(e.failed_runs||0)>0||_>=50||I>=.4?"High":Number(e.mismatch_runs||0)>0?"Medium":"Low"})(),j=(()=>{let m=Math.max(1,Number(e.attempted_runs||e.requested_grains||1)),I=Number(e.successful_runs||0),F=Number(e.failed_runs||0),L=c.length,O=55;return I>0&&(O+=18),F===0&&(O+=12),O+=Math.min(10,Math.round(L/m*10)),Math.max(35,Math.min(95,O))})(),k=e.verdict==="mismatch"?H==="High"?"Block release: review Issues now and review high-priority mismatches before accepting these targets as correct.":"Needs attention: review mismatch rows and mark expected changes in review, then resolve the rest.":e.verdict==="error"?"Block release: fix run errors first, then re-run upload analysis.":"Safe for now: targets look healthy. Continue with scheduled monitoring checks.",P=N.length?N.join(" \u2022 "):p.length>0?`${p.length} KPI check(s) need deeper review.`:"No major KPI shifts detected.",v=`
    ...
    ${o.length?`
        ...
        `:""}
  `,C=(e.errors||[]).map(m=>m?.error).filter(m=>!!m).slice(0,3),R=e?.issues_sync&&typeof e.issues_sync=="object"?e.issues_sync:null,M=R?.auto_enabled!==!1,E=Number(R?.created||0),x=Number(R?.updated||0),y=Number(R?.candidates||0),T=R?E>0?`<div class="tiny" style="margin-top:10px; color:var(--ok);">Added ${u(E)} mismatch issue${E===1?"":"s"} to <strong>Issues</strong> automatically.</div>`:x>0?`<div class="tiny muted" style="margin-top:10px;">Refreshed ${u(x)} existing mismatch issue${x===1?"":"s"} in <strong>Issues</strong>.</div>`:y>0?'<div class="tiny muted" style="margin-top:10px;">Upload mismatches are already present in <strong>Issues</strong>.</div>':'<div class="tiny muted" style="margin-top:10px;">No mismatch rows to add to <strong>Issues</strong> from this run.</div>':"",$=r.length&&(!R||!M)?`
    ...
    `:"",f=r.length?'<div class="tiny muted" style="margin-top:8px;">If the Issues list looks empty, clear Status/Severity filters. The button above opens Issues with filters reset.</div>':"",g=C.length?`
      <div class="upload-analysis-errors">
        <strong>Upload analysis errors</strong>
        <div class="history-list" style="margin-top:8px;">
          ${C.map(m=>`<div class="history-item upload-analysis-error-item">${S(m)}</div>`).join("")}
        </div>
      </div>
    `:"";return`
    <div class="results-auto-show">
      ...
      <div class="upload-analysis-summary-grid">
        ...
        <div class="meta-card meta-card-compact">
          ...
          <span>${H}</span>
          <div class="tiny muted" style="margin-top:4px;">Confidence ${u(j)}%</div>
        </div>
      </div>
      <div class="row-card" style="margin-top:12px;">
        <strong>Recommended next step</strong>
        <div class="tiny" style="margin-top:6px;">${S(k)}</div>
        <div class="tiny muted" style="margin-top:8px;">Impact focus: ${S(P)}</div>
      </div>
      ${$}
      ${V}
      ${f}
  ...
  `;document.querySelectorAll("#nav button[data-feature]").forEach(P=>{let v=P.dataset.feature;if(v&&!wt(v)){P.classList.add("feature-locked");let C=P.querySelector(".nav-label");C&&!C.querySelector(".feature-lock-badge")&&(C.innerHTML+=' <span class="feature-lock-badge" style="font-size:7px; vertical-align:middle; margin-left:4px;">BUSINESS</span>')}else P.classList.remove("feature-locked")});let c=s.status?.endpoints||[],p=$t(),_=Di(),A=Yi(p,_),N=Qi(_),H=Ki(p);if(p.length){let P=_==="table"?"":(()=>{let v=p.reduce((M,E)=>{let x=lt(E.endpoint_path);return M[x]=M[x]||[],M[x].push(E),M},{}),C=Object.entries(v).sort((M,E)=>M[0].localeCompare(E[0])),R=p.length>24;return C.map(([M,E])=>{let x=s.collapsedGroups[M]??(_==="grouped"&&(R||E.length>8));return er(M,E,{compact:_==="compact",collapsed:x})}).join("")})();a.apiList.innerHTML=l+d+A+H+N+`
            ${_==="table"?Gi(p):`<div class="api-browser-body ${_==="compact"?"compact":"grouped"}">${P}</div>`}
          ...
          `}else{let P=!!((s.apiFilter||"").trim()||(s.apiStatusFilter||"").trim()),v=c.length===0&&!P?`
      ...
      `:"",C=P?"No APIs match this search.":c.length===0?s.apiDataState==="auth_required"?"Your Jin session expired. Sign in again to load APIs.":s.apiDataState==="error"?"Jin returned an error while loading APIs. Check server logs and retry.":s.apiDataState==="unavailable"?"Cannot load APIs right now. Check backend connection and retry.":"No APIs connected yet. Open Overview for the first-run checklist.":"No APIs match this search.";a.apiList.innerHTML=l+d+A+H+N+(v||ke(C))}let j=a.apiList.querySelector(".api-browser-table-body.virtualized");if(j){let P=window.handleApiBrowserTableScroll;typeof P=="function"&&j.addEventListener("scroll",P,{passive:!0}),j.scrollTop=Math.max(0,Number(s.apiBrowserScrollTop||0))}let k=window.syncApiBrowserPinnedOffsets;typeof k=="function"&&k()}window.showRunDetail=e=>{let t=s.activeApiDetail;if(!t)return;let i=((t.history&&t.history.length?t.history:t.recent_history)||[]).filter(g=>g.observed_at===e);if(a.runDetailTitle.innerText=`Run Detail: ${ne(e)}`,a.runDetailDrawer.style.display="block",!i.length){a.runDetailContent.innerHTML=ke("No observations for this timestamp.");return}let r=g=>{if(!g)return{};if(typeof g=="string")try{let m=JSON.parse(g);return m&&typeof m=="object"?m:{}}catch{return{}}return typeof g=="object"?g:{}},o=g=>{let m=new Map,I=(F,L="")=>{if(F!=null){if(typeof F=="number"){L&&Number.isFinite(F)&&!m.has(L)&&m.set(L,F);return}if(typeof F=="string"){let O=F.trim(),U=Number(O);L&&O&&Number.isFinite(U)&&!m.has(L)&&m.set(L,U);return}if(Array.isArray(F)){F.forEach(O=>I(O,L?`${L}[]`:"data[]"));return}typeof F=="object"&&Object.entries(F).forEach(([O,U])=>{let D=L?`${L}.${O}`:O;I(U,D)})}};return I(g),Array.from(m.entries()).map(([F,L])=>({kpiField:F,value:L}))},l=new Map;(t.references||[]).forEach(g=>{let m=String(g?.grain_key||""),I=String(g?.kpi_field||"");if(!m||!I)return;let F=g?.expected_value,L=g?.tolerance_pct,O=F==null?null:Number(F),U=L==null?null:Number(L),D={expected:Number.isFinite(O)?O:null,tolerance:Number.isFinite(U)?U:null},le=Rt(I);l.set(`${m}__${I}`,D),le&&le!==I&&l.set(`${m}__${le}`,D);let Y=Ke(m);Y&&(l.has(`${Y}__${I}`)||l.set(`${Y}__${I}`,D),le&&!l.has(`${Y}__${le}`)&&l.set(`${Y}__${le}`,D))});let d=Number(t?.config?.tolerance_normal??t?.config?.tolerance_pct??10),p=i.flatMap(g=>{let m=String(g?.grain_key||"Global"),I=Array.isArray(g?.comparisons)?g.comparisons.filter(L=>L?.kpi_field):[];if(I.length)return I.map(L=>{let O=L.actual_value??L.actual,U=String(L.kpi_field||""),D=Rt(U),le=Ke(m),Y=l.get(`${m}__${U}`)||(D?l.get(`${m}__${D}`):void 0)||l.get(`${le}__${U}`)||(D?l.get(`${le}__${D}`):void 0),pe=L.expected_value??L.expected??Y?.expected??null,Ae=L.allowed_tolerance_pct??Y?.tolerance??d,K=pe==null?null:Number(pe),se=O==null?null:Number(O),be=L.pct_change!=null?Number(L.pct_change):K==null||se==null||K===0?null:(se-K)/Math.abs(K)*100,qe=String(L.status||""),b=qe&&!(qe==="missing_reference"&&K!=null)?qe:K==null?"missing_reference":be!=null&&Math.abs(be)>Ae?"mismatch":"match";return{grainKey:m,kpiField:String(L.kpi_field),actual:se,expected:K,pctChange:be,status:b,message:L.message||"Historical comparison for this metric."}});let F=r(g?.kpi_json);return o(F).map(({kpiField:L,value:O})=>{let U=Number(O),D=Rt(L),le=Ke(m),Y=l.get(`${m}__${L}`)||(D?l.get(`${m}__${D}`):void 0)||l.get(`${le}__${L}`)||(D?l.get(`${le}__${D}`):void 0),pe=Y?.expected??null,Ae=Y?.tolerance??d,K=pe==null||pe===0?null:(U-pe)/Math.abs(pe)*100,se=pe==null?"missing_reference":K!=null&&Math.abs(K)>Ae?"mismatch":"match",be=pe==null?"No uploaded targets were found for this metric on this grain.":`Derived from run observation and uploaded targets (tolerance +/-${Ae.toFixed(1)}%).`;return{grainKey:m,kpiField:L,actual:U,expected:pe,pctChange:K,status:se,message:be}})}).filter(g=>g&&String(g.kpiField||"").trim().length>0);if(!p.length){let g=i.flatMap(I=>Object.keys(r(I?.kpi_json)||{})).filter((I,F,L)=>L.indexOf(I)===F),m=g.length?`Captured keys: ${g.slice(0,6).join(", ")}${g.length>6?", ...":""}.`:"No KPI values were returned for this run.";a.runDetailContent.innerHTML=ke(`No comparable KPI values were captured for this run. ${m}`);return}let _=new Set(["api_version","timestamp","_jin_id"]),A=g=>{if(!g||g.indexOf("|")===-1)return{};let[,...m]=g.split("|"),I={};return m.forEach(F=>{let L=F.indexOf("=");if(L<=0)return;let O=F.slice(0,L),U=F.slice(L+1);I[O]=U}),I},N=g=>{let m=A(g),I=m.retailer||m.grain_retailer,F=m["data[].date"]||m.date||m.period,L=m["data[].label"]||m.label,O=[I?`Retailer: ${I}`:null,F?`Date: ${F}`:null,L?`Label: ${L}`:null].filter(Boolean);if(O.length)return O.join(" \u2022 ");let U=Object.entries(m).filter(([D])=>!_.has(D)).slice(0,3).map(([D,le])=>`${D}=${le}`);return U.length?U.join(" \u2022 "):"Global grain"},H=new Map;p.forEach(g=>{let m=String(g.grainKey||"Global");H.has(m)||H.set(m,[]),H.get(m).push(g)});let j=Array.from(H.entries()).map(([g,m])=>{let I=m.filter(L=>L.status!=="match"&&L.status!=="missing_reference").length,F=m.every(L=>L.status==="missing_reference"||L.expected==null);return{grainKey:g,title:N(g),rows:m,needsReviewCount:I,missingBaseline:F,status:I>0?"mismatch":F?"missing_reference":"match"}}).sort((g,m)=>(g.status==="match"?1:0)-(m.status==="match"?1:0)),k=j.length,P=j.filter(g=>g.status==="mismatch"),v=j.filter(g=>g.status==="missing_reference"),C=v.length,R=P.length,M=j.filter(g=>g.status==="match"),E=g=>{let m=g.rows.filter(O=>O.status==="mismatch"||O.status==="missing_kpi"||O.status==="error").length,I=g.rows.filter(O=>O.status==="missing_reference").length,F=g.rows.filter(O=>O.status==="match").length,L=[`${u(g.rows.length)} metric(s)`];return m&&L.push(`${u(m)} need attention`),I&&L.push(`${u(I)} without targets`),F&&L.push(`${u(F)} matched`),L.join(" \u2022 ")},x=g=>{let m=Xe(String(g.kpiField||"metric")),I=g.pctChange==null?null:Number(g.pctChange);if(g.status==="match")return I==null?`${m} is within the allowed range.`:`${m} is within the allowed range (${Math.abs(I).toFixed(1)}% change).`;if(g.status==="missing_reference")return`No targets are uploaded for ${m}.`;if(g.status==="missing_kpi")return`${m} was not returned by the API for this grain.`;if(g.status==="error")return g.message||`Could not evaluate ${m}.`;if(I==null)return`${m} is outside the allowed range.`;let F=I>0?"higher":"lower";return`${m} is ${Math.abs(I).toFixed(1)}% ${F} than target (outside tolerance).`},y=g=>{let m=g.rows.filter(D=>D.status!=="match"),I=g.rows.filter(D=>D.status==="match"),F=m.length?m:I.slice(0,3),L=m.length?I:I.slice(3),O=m.slice(0,2).map(D=>x(D)).join(" \u2022 "),U=Math.max(0,m.length-2);return`
    <div class="upload-analysis-card upload-analysis-card-${g.status==="match"?"match":g.status==="missing_reference"?"setup":"mismatch"} run-detail-grain-card">
      <div class="upload-analysis-header">
        <div>
          ...
          <div class="tiny muted">${S(E(g))}</div>
        </div>
        <span class="status-pill ${It(g.status)}">${S(ht(g.status))}</span>
      </div>

      <div class="run-detail-card-stats tiny">
        ${u(g.rows.length)} metric(s) \u2022 ${u(m.length)} need attention \u2022 ${u(I.length)} within range
      </div>
      ...
      <details class="run-detail-kpi-breakdown">
        <summary>${m.length?`${u(m.length)} KPI(s) need attention`:`${u(g.rows.length)} KPI(s) within targets`}</summary>
        ...
        ${L.length?`
          <details class="upload-analysis-inline-more">
            <summary>Show ${u(L.length)} additional matched KPI(s)</summary>
            <div class="table-wrap" style="margin-top:10px;">
              <table class="row-table run-detail-kpi-table">
                <thead>
                  <tr>
  ...
  `},T=P.slice(0,6),$=P.slice(6),V=v.slice(0,6),f=v.slice(6);a.runDetailContent.innerHTML=`
    ...
    <div class="history-list" style="margin-top:14px;">
      ${P.length?`<div class="upload-analysis-section-title">Needs attention (${u(P.length)})</div>`:""}
      ${T.map(g=>y(g)).join("")}
      ${$.length?`
        <details class="upload-analysis-more-runs">
      ...
      `:""}
      ${v.length?`<div class="upload-analysis-section-title">Needs target setup (${u(v.length)})</div>`:""}
      ${V.map(g=>y(g)).join("")}
      ${f.length?`
        <details class="upload-analysis-more-runs">
          <summary>Show ${u(f.length)} more grain(s) needing target setup</summary>
          <div class="history-list" style="margin-top:12px;">
            ${f.map(g=>y(g)).join("")}
          </div>
        </details>
      `:""}
      ${!P.length&&!v.length?'<div class="upload-analysis-section-title">No grains need attention</div>':""}
      ${M.length?`
        <details class="upload-analysis-more-runs upload-analysis-matched-group">
          <summary>Matched grains (${u(M.length)})</summary>
          <div class="history-list" style="margin-top:12px;">
            ${M.map(g=>y(g)).join("")}
          </div>
        </details>
  ...
  `};function Fs(){let e=s.status?.endpoints||[],t=s.anomalies?.anomalies||[],n=s.status?.summary||{total_endpoints:0,healthy:0,anomalies:0,unconfirmed:0},i=s.status?.project,r=s.status?.recent_errors||[],o=e.length?Math.round((n.healthy||0)/e.length*100):100;a.overviewMetrics.innerHTML=[tt("Healthy",`${o}%`,"Healthy right now"),tt("At risk",t.length+Number(n.unconfirmed||0),"APIs to review or finish")].join(""),a.overviewCharts.innerHTML=`
    ...
    <div class="row-card playbook-next-card" style="margin-bottom:12px;">
      ...
      <div class="toolbar" style="margin-top:10px; gap:8px;">
        <button class="action" type="button" data-view="${_}">${A}</button>
  ...
  `,Ms()}function Ms(){let e=Me(),t=document.getElementById("playbook-maintainer-setup"),n=document.getElementById("playbook-core-workflow");if(t&&(t.style.display=e?"":"none"),n&&(n.style.display=e?"":"none"),!e){[a.projectWorkflowFeedback,a.projectWorkflowHealth,a.projectWorkflowMonitor,a.projectWorkflowRuns,a.projectWorkflowReport].forEach(f=>{let g=f.closest(".row-card");g&&(g.style.display="none")});return}let i=s.projectsCatalog||[],o=String(a.projectActiveSelect.value||"").trim()||s.activeProjectId||i.find(f=>f.active&&!f.is_archived)?.id||i.find(f=>!f.is_archived)?.id||i[0]?.id||"",l=i.find(f=>String(f.id)===String(o)),d=!!l?.is_archived,c=Array.isArray(s.status?.endpoints)?s.status.endpoints:[],p=c.length,_=c.filter(f=>!!f?.last_upload_at).length,N=(Array.isArray(s.scheduler?.jobs)?s.scheduler.jobs:[]).filter(f=>{let g=String(f?.job_id||"");if(!g||g.startsWith("jin:bundle:"))return!1;let m=String(f?.job_type||"").toLowerCase();if(m&&m!=="watch"||!String(f?.endpoint_path||f?.path||"").trim())return!1;let F=String(f?.skip_reason||"");return F!=="missing_default_params"&&F!=="unsupported_schedule"}),H=Array.isArray(s.projectRunHistory)&&s.projectRunHistory.length>0,j=String(a.projectDeleteConfirm.value||"").trim(),k=String(l?.name||"").trim(),P=!!k&&j===k;if(!String(a.projectRegisterName.value||"").trim()){let f=String(s.status?.project?.name||"").trim();f&&(a.projectRegisterName.value=f)}a.projectActiveSelect.innerHTML=i.length?i.map(f=>`
      <option value="${f.id}" ${f.id===o?"selected":""}>
        ${f.name}${f.active?" (active)":""}${f.is_archived?" [archived]":""}
      </option>
    `).join(""):'<option value="">No project found</option>';let v=String(s.activeProjectId||""),C=!!o&&o!==v&&!d;a.projectSelectButton.disabled=!C,a.projectSelectButton.textContent=d?"Archived (restore first)":C?"Switch to Selected Project":"Already Active",a.projectArchiveButton.disabled=!o||d,a.projectRestoreButton.disabled=!o||!d,a.projectDeleteButton.disabled=!o||!d||!P,a.projectDeleteConfirm.placeholder=k?`Type "${k}" to delete`:"Type selected project name exactly";let R=!o||d,M=p===0;if(a.projectPolicySaveButton.disabled=R||M,a.projectPolicyApplyButton.disabled=R||M,a.projectRunBundleButton.disabled=R||M,a.projectBaselinePromoteButton.disabled=R||M,a.projectHealthCheckButton.disabled=R,a.projectMonitorRefreshButton.disabled=R,a.projectReportDigestButton.disabled=R||M||!H,s.projectPolicyLoadedFor===o&&s.projectMonitorPolicy){let f=s.projectMonitorPolicy||{};a.projectPolicyCadence.value=String(f.cadence_template||"balanced"),a.projectPolicySchedule.value=String(f.schedule||"every 2h"),a.projectPolicyBaselineMode.value=String(f.baseline_mode||"fixed"),a.projectPolicyThreshold.value=f.threshold==null?"":String(f.threshold),a.projectPolicyBundleEnabled.checked=!!f.bundle_enabled,a.projectPolicyBundleSchedule.value=String(f.bundle_schedule||"daily 09:00"),a.projectPolicyBundleFormat.value=String(f.bundle_report_format||"markdown")}let E=R?d?"Selected project is archived. Restore it to continue.":"Create or select a project to begin.":M?"No APIs are tracked yet. Call your APIs once, then save setup and run checks.":_===0?`APIs are tracked (${u(p)}), but no targets are uploaded yet. Set Up APIs and upload target files.`:N.length===0?"Setup is saved but no runnable watch jobs are configured yet. Click Save & Apply Setup.":H?`Core workflow is ready: ${u(p)} API${p===1?"":"s"} tracked, ${u(_)} with targets, ${u(N.length)} runnable watches.`:"Setup is ready. Run checks now to create the first monitoring run.",x=s.projectWorkflowMessage||{text:E,kind:"info"};a.projectWorkflowFeedback.textContent=x.text||"",a.projectWorkflowFeedback.className=`feedback${x.kind==="error"?" danger":x.kind==="success"?" success":" info"}`;let y=s.projectHealth;if(!y)a.projectWorkflowHealth.innerHTML="";else{let f=y.checks||[];a.projectWorkflowHealth.innerHTML=`
      <div class="row-card">
    ...
    `}let T=s.projectsMonitorSnapshot;if(!T)a.projectWorkflowMonitor.innerHTML="";else{let f=T.projects||[],g=T.summary||{},m=g.top_risk_project||null,I=g.healthy_projects??f.filter(Y=>String(Y.status||"").toLowerCase()==="healthy").length,F=g.degraded_projects??f.filter(Y=>String(Y.status||"").toLowerCase()!=="healthy").length,L=g.projects_with_baseline??f.filter(Y=>Number(Y.baseline?.coverage_pct||0)>=70).length,O=g.average_risk_score??(f.length?f.reduce((Y,pe)=>Y+Number(pe.risk_score||0),0)/f.length:0),U=Fi({endpoints:f.length,anomalies:Number(m?.summary?.anomalies||0),needsSetup:Number(f.filter(Y=>Number(Y.baseline?.coverage_pct||0)<70).length),needsAttention:F,topRiskScore:Number(m?.risk_score||0)}),D=[tt("Projects",u(T.count||f.length),"Tracked across the portfolio"),tt("Healthy",u(I),"Projects currently stable"),tt("At risk",u(F),"Projects needing attention"),tt("Avg risk",`${Math.round(Number(O||0))}%`,"Portfolio risk snapshot")].join(""),le=m?U.label==="Block release"?"Treat this portfolio as blocked until the highest-risk project is addressed.":U.label==="Needs attention"?"Review the highest-risk project first, then compare it with the rest of the portfolio.":"The portfolio looks safe for now. Keep watching the highest-risk project and recheck weekly.":"Keep the portfolio view open while you review project health and target coverage.";a.projectWorkflowMonitor.innerHTML=`
      <div class="row-card">
        <strong>Portfolio Health</strong>
        <div class="tiny" style="margin-top:6px;">
          Projects: ${u(T.count||f.length)} \u2022 Target coverage: ${u(L)} \u2022 Generated: ${ne(T.generated_at)}
        </div>
        ...
        ${m?`
          <div class="sidebar-card" style="margin-top:12px;">
            ...
            <div class="tiny" style="margin-top:6px;">${S(String(m.name||"Unknown project"))} \u2022 ${U.label} \u2022 risk ${Math.round(Number(m.risk_score||0))}%</div>
    ...
    `:"";let V=s.projectDigest;if(!V)a.projectWorkflowReport.innerHTML="";else{let f=V.totals||{};a.projectWorkflowReport.innerHTML=`
      <div class="row-card">
        <strong>Leadership Digest (${u(V.window_days||7)}d)</strong>
        <div class="tiny" style="margin-top:6px;">
    ...
    `}}function Ve(){let e=Bt(De()),t=nt(),n=ct(t,s.incidentPage,10);s.incidentPage=n.page;let i=s.incidentsMessage;a.incidentsFeedback.textContent=i?.text||"",a.incidentsFeedback.className=`feedback feedback-banner${i?.kind==="error"?" danger":i?.kind==="success"?" success":i?" info":""}`;let r=e.filter(f=>String(f.status||"active")!=="resolved").length,o=e.filter(f=>String(f.status||"active")!=="resolved"&&["critical","high"].includes(String(f.severity||"").toLowerCase())).length,l=t.find(f=>String(f.status||"active")!=="resolved")||e.find(f=>String(f.status||"active")!=="resolved")||null,d=document.querySelectorAll(".bulk-incident:checked").length,c=d>0;a.bulkPreview.textContent=c?`${d} issue${d===1?"":"s"} selected.`:"Select one or more issues to apply one action.",a.bulkAction.style.display=c?"":"none",a.bulkNote.style.display=c?"":"none",a.bulkRun.style.display=c?"":"none",a.incidentFilters.innerHTML=`
      ...
      `;let p=f=>{let g=String(f||"active").toLowerCase();return g==="acknowledged"?"In review":g==="snoozed"?"Deferred":g==="suppressed"?"Muted":g==="resolved"?"Resolved":"Needs attention"},_=f=>{let g=String(f||"medium").toLowerCase();return g==="critical"||g==="high"?"High":g==="low"?"Low":"Medium"},A=f=>{let g=String(f||"active").toLowerCase();return g==="resolved"?"resolved":g==="acknowledged"?"acknowledged":"active"},N=f=>{let g=ns(f);return g==="danger"?"danger":g==="warning"?"warning":"healthy"},H=f=>{let g=Number(f);if(!Number.isFinite(g))return null;let m=g<=1?g*100:g,I=Math.max(0,Math.min(100,m));return`${Math.round(I)}% confidence`},j=(f,g=120)=>{let m=String(f||"").trim();return m?m.length<=g?m:`${m.slice(0,g-1).trimEnd()}\u2026`:""},k=f=>{let g=Number(f?.baseline_used),m=Number(f?.actual_value),I=Number.isFinite(g)?g:null,F=Number.isFinite(m)?m:null;if(I==null||F==null)return`Expected ${u(f?.baseline_used)} -> Actual ${u(f?.actual_value)}`;if(I===0)return`Expected 0 -> Actual ${u(F)} (target is zero)`;let L=Math.abs(F/I),O=L>=100?`${L.toFixed(0)}x`:`${L.toFixed(1)}x`;return`Expected ${u(I)} -> Actual ${u(F)} (${O} vs target)`},P=!!((s.incidentStatusFilter||"").trim()||(s.incidentSeverityFilter||"").trim()),v=[];s.incidentStatusFilter&&v.push(`Status: ${p(s.incidentStatusFilter)}`),s.incidentSeverityFilter&&v.push(`Priority: ${_(s.incidentSeverityFilter)}`);let C=v.join(" \u2022 "),R=t.length?(n.page-1)*10+1:0,M=t.length?R+n.items.length-1:0,E=`
  ...
  `,x=P?`
      <div class="row-card issue-filter-summary" style="margin-bottom:10px;">
        ...
        <div class="tiny" style="margin-top:6px;">
          Showing ${u(t.length)} issue${t.length===1?"":"s"} with ${S(C)}.
        </div>
        <div class="toolbar" style="margin-top:10px;">
          <button class="action secondary" type="button" onclick="clearIncidentFilters()">Clear filters</button>
        </div>
      </div>
    `:"",y=s.incidentSort==="business",T=P?`Showing ${u(R)}-${u(M)} of ${u(t.length)} after filters.`:`Showing ${u(R)}-${u(M)} of ${u(t.length)} in priority order.`,$=l?`<button class="action" type="button" onclick="showIncident(${l.id})">Review Top Issue</button>`:'<button class="action" type="button" data-view="api">Set Up APIs</button>',V=t.length===0&&e.length>0&&P;a.incidentsList.innerHTML=t.length?`
        ${x}
        ${E}
        <div class="row-card issue-queue-summary" style="margin-bottom:10px;">
          <strong>Issue Review Queue</strong>
          <div class="tiny" style="margin-top:6px;">${T}</div>
              ...
              <div class="issue-card-body">
                <div class="issue-card-header">
                  ...
                  <div class="issue-card-priority">
                    ...
                    <span class="status-pill ${A(f.status)}">${p(f.status)}</span>
                ...
                <div class="issue-card-meta">
                  ...
                  <div class="issue-card-meta-item">
                    ...
                    <span class="tiny muted issue-card-meta-value">${Lt(f)} \u2022 ${p(f.status)} \u2022 ${_(f.severity)} priority</span>
                  </div>
                </div>
      ...
      `:`
        <div class="empty empty-center issue-empty-state">
          ...
          <div class="toolbar" style="margin-top:12px; justify-content:center;">
            ${P?'<button class="action secondary" type="button" onclick="clearIncidentFilters()">Clear filters</button>':""}
            <button class="action" type="button" data-view="api">Set Up APIs</button>
          </div>
        </div>
  ...
  `,C=N?`
    ...
    `:"",R=!!H?.ready,M=!!(H&&H.candidateCount>0),E=new Map,x=(b,B)=>{let W=String(b||"").trim();!W||E.has(W)||E.set(W,B)};H&&(x(H.timeCandidates?.[0],"time"),x(H.segmentCandidates?.[0],"dimension"),x(H.metricCandidates?.[0],"kpi"));let y=N?"":R?`
         ...
         <div style="margin-top:12px; display:flex; flex-direction:column; gap:8px;">
            ${Object.entries(s.driftSuggestions).map(([b,B])=>`
               <div style="display:flex; align-items:center; gap:10px; background:rgba(255,255,255,0.05); padding:8px 12px; border-radius:8px; border:1px solid var(--line);">
                  ...
                  <button class="btn btn-xs btn-primary" style="margin-left:auto;" onclick="approveDriftMerge('${b}', '${B}')" ${p?'disabled title="Turn off PO Mode to merge manually."':""}>Approve & Merge</button>
               </div>
            `).join("")}
         </div>
      </div>
    `);let V=String(s.selectedApi||""),f=!!s.configFocusExpandedByApi?.[V],g=(b,B,W)=>{if(W)return!0;let q=String(b||"").toLowerCase(),ee=String(B||"").toLowerCase();return ee==="date"||ee==="datetime"||ee==="int"||ee==="float"||ee==="decimal"?!0:/(retailer|merchant|store|region|country|channel|category|segment|sku|product|item|label|group|period|date|time|timestamp|value|amount|revenue|sales|orders|units|count|qty|quantity|cost|rate|ratio|score)/.test(q)},m=(e||[]).map(b=>{let B=typeof b=="string"?b:String(b.name||"");if(!B)return null;let W=Ls(B),q=W!==B,ee=typeof b=="string"?"":b.annotation||b.type||"",z=mr(ee),te=typeof b=="string"?!1:!!(b.time_candidate||String(b.suggested_role||"").toLowerCase()==="time"),Se=E.get(B)||null,ye=(t.excluded_fields?.includes(B)?"exclude":null)||(r.has(B)?"dimension":null)||(o.has(B)?"kpi":null)||(B===l?"time":null)||(te?"time":null)||Se||"ignore",J=typeof b=="string"?!1:!!b.suggested;return{name:B,shownName:W,hasTechnicalPath:q,rawType:ee,type:z,role:ye,suggested:J,timeCandidate:te,likely:g(B,ee,J)}}).filter(b=>!!b),I=(A||_)&&m.length>8&&!f,F=I?m.filter(b=>b.role!=="ignore"||b.likely):m,L=F.length?F:m,O=Math.max(0,m.length-L.length),U=I?`
      <div class="row-card" style="margin-bottom:12px;">
        <strong>Focused view</strong>
        <div class="tiny" style="margin-top:6px;">
          Showing ${u(L.length)} likely business fields first to reduce setup noise.
  ...
  `;let K=k.length?`for every unique <strong>${k.join(", ")}</strong>`:"across your entire dataset",se=P.length?`monitor <strong>${P.join(" and ")}</strong>`:"track any numbers yet",be=document.getElementById("config-step-3-container");if(be){let b=P.length>0||k.length>0||l,B=String(s.selectedApi||""),W=s.autoSuggestSummaryByApi||{},q=B?W[B]:null,ee=Es(s.activeApiDetail),z=i.response_model_present===!1,te=z?null:ot(i.fields||i.schema_contract?.fields||[]),Se=!!te?.ready,ye=!!(te&&te.candidateCount>0),J=z?"No response_model detected. Jin can infer Segment, Metric, and Time from runtime traffic or a pasted sample payload.":q?`${q.headline} ${q.details}`:Se?"Jin can pre-fill Segment, Metric, and Time from the Pydantic response model even before traffic arrives.":ee?`Use auto-suggest to pre-fill setup from recent API traffic or Pydantic examples. ${te?.detail||""}`:`${te?.summary||"The response model is present, but Jin needs clearer typed fields or examples to pre-fill setup."} ${te?.detail||""}`.trim();be.innerHTML=`
      <div class="config-story-card ${b?"active":""}" style="margin-top:24px; border-top:1px solid var(--line); padding-top:24px;">
        ...
        <div class="tiny muted" style="margin-top:12px;">
          ${P.length&&k.length?"This is a great setup! You'll see trends broken down by detailed segments.":"Tip: Select at least one group and one measurable number for the best results."}
        </div>
  ...
  `;let x=(s.status?.endpoints||[]).find(b=>b.endpoint_path===e.endpoint_path),y=e.response_model_present===!1,T=y?{label:"Define response model",tone:"danger",hint:"Add response_model on this FastAPI route first. Jin uses it to discover fields and time candidates."}:st({endpoint_path:e.endpoint_path,http_method:e.http_method||"GET",status:x?.status||"warning",dimension_fields:x?.dimension_fields||e.config?.dimension_fields||[],kpi_fields:x?.kpi_fields||e.config?.kpi_fields||[],time_field:x?.time_field||e.config?.time_field||null,time_required:x?.time_required??e.config?.time_required,confirmed:e.operator_metadata?.confirmed??x?.confirmed??!1,last_upload_at:r.last_upload_at||x?.last_upload_at||null});a.apiMetaGrid.innerHTML=[["Status",`
    ...
    `],["Setup",`
      ...
      <div class="tiny muted" style="margin-top:4px;">${S(T.hint)}</div>
    `],["Activity",`
      <strong>${u(r.observation_count)} checks</strong>
      <div class="tiny muted" style="margin-top:4px;">Last upload ${ne(r.last_upload_at)}</div>
    `],["Issues",A>0?`<span class="status-pill warning">${u(A)} open</span><div class="tiny muted" style="margin-top:4px;">Review Issues</div>`:'<span class="status-pill healthy">Safe for now</span><div class="tiny muted" style="margin-top:4px;">No open issues</div>']].map(([b,B])=>`
        <div class="meta-card meta-card-compact">
          <strong>${b}</strong>
          <span>${B}</span>
        </div>
      `).join("");let $=(s.coreInsightsByApi||{})[e.endpoint_path];if($){let b=$.kind==="error"?"danger":$.kind==="success"?"success":"info",B="";$.actionType==="tab"&&$.actionValue?B=`<button class="action" type="button" onclick="switchApiTab('${S($.actionValue)}')">${S($.actionLabel||"Open next step")}</button>`:$.actionType==="view"&&$.actionValue&&(B=`<button class="action" type="button" data-view="${S($.actionValue)}">${S($.actionLabel||"Open next step")}</button>`),a.apiCoreInsight.style.display="block",a.apiCoreInsight.innerHTML=`
    ...
    `}else a.apiCoreInsight.style.display="none",a.apiCoreInsight.innerHTML="";let V=e.current_kpis||[];a.apiKpis.innerHTML=V.length?V.map(b=>`
        <div class="kpi-card">
          ...
          <span>${u(b.actual_value)}</span>
          <div class="delta">${b.expected_value==null?"No targets yet. Upload a reference to compare.":`Target ${u(b.expected_value)}${b.pct_change==null?"":` \u2022 ${u(b.pct_change)}% vs target`}`}</div>
        </div>
      `).join(""):`
        <div class="empty empty-starter">
          ${d?P?"Recent API values are available below. Expand Monitor runs to see segment-level comparisons.":"Recent API values are available below, but no targets are linked yet. Upload a reference file to enable pass/fail checks.":"No values yet. Upload a reference file and run a check to get started."}
        </div>
      `;let f=e.trend_summary||[],g=e.recent_history||[];a.apiTrends.innerHTML=f.length?f.map(b=>{let B=g.map(q=>q?.kpi_json?.[b.kpi_field]).filter(q=>typeof q=="number"),W=B.length?B.reduce((q,ee)=>q+ee,0)/B.length:null;return`
          <div class="trend-card">
            <strong>${ft(b.kpi_field)}</strong>
            <span>${u(b.latest)}</span>
            <div class="tiny">Typical range: ${u(b.min)} to ${u(b.max)} across ${b.samples} run(s).</div>
            <div class="tiny" style="margin-top:6px;">Average: ${u(W)} \u2022 Change vs first visible run: ${u(b.delta_pct)}%.</div>
            ${ks(B)}
          </div>
      ...
      `;let m=document.getElementById("api-start-panel");if(m){let b=!!v,B=b&&!!R,W=B&&!!_,q=W&&A===0,ee=s.currentApiTab==="summary";m.style.display=ee?"block":"none";let z=(Se,ye)=>Se?'<span class="status-pill healthy">Done</span>':ye?'<span class="status-pill warning">Now</span>':'<span class="status-pill acknowledged">Next</span>',te=b?B?W?A>0?{title:"Step 4: Review issues",copy:"Some checks are outside target. Review and resolve what changed.",cta:"Review Issues",action:"setView('incidents')"}:{title:"Step 5: Generate report",copy:"Monitoring is stable. Generate a leadership-ready update.",cta:"Open Reports",action:"setView('reports')"}:{title:"Step 3: Monitor",copy:"Run checks and see pass/fail status for each segment.",cta:"Open Monitor",action:"switchApiTab('history')"}:{title:"Step 2: Upload targets",copy:"Upload a reference file so checks have a clear target.",cta:"Open Uploads",action:"switchApiTab('uploads')"}:{title:"Step 1: Identify segments & metrics",copy:"Define which fields are segments and which are monitored metrics.",cta:"Open Configure",action:"switchApiTab('configuration')"};m.innerHTML=`
        ...
        `}let I=s.currentApiTab;a.configFooter&&(a.configFooter.style.display=I==="configuration"&&v?"flex":"none"),a.uploadsFooter&&(a.uploadsFooter.style.display=I==="uploads"?"flex":"none"),a.summaryFooter&&(a.summaryFooter.style.display=I==="summary"?"flex":"none");let F=a.apiKpis.closest(".panel"),L=a.apiIncidentHistory.closest(".panel"),O=a.apiRunTable.closest(".panel");F&&(F.style.display=l||d||M===3?"block":"none"),L&&(L.style.display=c||M===3?"block":"none"),O&&(O.style.display=_||M===3?"block":"none");let U=document.getElementById("api-monitoring-progress");if(U){let b=U.dataset.endpoint,B=U.dataset.source;k?(U.dataset.endpoint=e.endpoint_path,U.dataset.source="upload-analysis",U.innerHTML=ur(k,e),U.style.display="block"):!_&&M===3?(U.dataset.endpoint=e.endpoint_path,U.dataset.source="empty-analysis",U.innerHTML=ke("Upload targets and run analysis to see per-segment results here."),U.style.display="block"):(b!==e.endpoint_path||B==="upload-analysis"||B==="empty-analysis")&&(U.dataset.endpoint=e.endpoint_path,U.dataset.source="",U.innerHTML="",U.style.display="none")}let D=e.anomaly_history||[],le=b=>{let B=Number(b?.actual_value),W=Number(b?.baseline_used);if(Number.isFinite(B)&&Number.isFinite(W)&&W!==0){let q=(B-W)/Math.abs(W)*100,ee=q>=0?"higher":"lower";return`API returned ${u(B)}; target is ${u(W)} (${Math.abs(q).toFixed(1)}% ${ee}).`}return Number.isFinite(B)?`API returned ${u(B)}. Target is not available for comparison.`:b.why_flagged||b.ai_explanation||"No details yet."};if(a.apiIncidentHistory.innerHTML=D.length?D.slice(0,10).map(b=>{let B=St(b),W=b.status||b.severity||"active",q=b.change_since_last_healthy_run||"No earlier comparison details.";return`
        <div class="history-item api-issue-card api-issue-card-${B}">
          ...
          <div class="api-issue-card-meta tiny muted">
            Detected ${ne(b.detected_at)} \u2022 Target ${b.baseline_used==null?"not set":u(b.baseline_used)}
          </div>
      ...
      `}).join(""):ke("No issues for this API yet."),o.length){let b=ct(o,s.runPage,8);s.runPage=b.page;let B=o.filter(z=>String(z?.trigger||"").toLowerCase()==="manual").length,W=o.filter(z=>String(z?.trigger||"").toLowerCase()==="scheduler").length,q=Number(k?.mismatch_runs||0),ee=o.filter(z=>{let te=String(z?.status||"").toLowerCase();return te==="error"||te==="failed"}).length;a.apiRunTable.innerHTML=`
        ...
        <div class="table-toolbar">
          ...
          <div class="tiny">Manual: ${u(B)} \u2022 Scheduled: ${u(W)} \u2022 Failed: ${u(ee)}</div>
        </div>
                ...
                <tr>
                  ...
                  <td>
                    ...
                    <div class="tiny muted" style="margin-top:6px;">
                      ${u(b.requested_grains)} segments \u2022 ${u(b.matched_runs)} matched \u2022 ${u(b.mismatch_runs)} mismatched \u2022 ${u(b.failed_runs)} errors
                    </div>
                  </td>
                  ...
                  <div class="upload-event-metrics">
                    ${b.metrics.slice(0,2).map(B=>`
                      <div class="upload-event-metric">
                        ...
                        <span>${u(B.expected_value)}</span>
                      </div>
                    `).join("")}
                    ${b.metrics.length>2?`
                      <details class="upload-analysis-inline-more">
                        <summary>Show ${u(b.metrics.length-2)} more metric(s)</summary>
      ...
      `:`${qe}${ke("No target rows are stored for this API yet.")}`,_e(e.fields||[],e.setup_config||e.config||{},e.metrics||[],e),Le(),Ct(),s.currentApiTab==="configuration"&&typeof window.refreshTimePreview=="function"&&requestAnimationFrame(()=>{window.refreshTimePreview()}),s.currentApiTab==="configuration"&&typeof window.runMagicGuess=="function"){let b=!!((e.setup_config?.dimension_fields||[]).length||(e.setup_config?.kpi_fields||[]).length||e.setup_config?.time_field),B=Es(e),W=String(e.endpoint_path||s.selectedApi||"").trim(),q=s.autoSuggestTriggeredByApi||{};!b&&B&&W&&!q[W]&&!y&&(q[W]=!0,s.autoSuggestTriggeredByApi=q,requestAnimationFrame(()=>{window.runMagicGuess(!1)}))}}function Vs(){let e=s.status?.project,t=Me(),n=e?.policy,i=String(e?.tier||"free").toLowerCase(),o=String(e?.license_backend||(e?.license_catalog_present?"commercial_catalog":"legacy_demo"))==="commercial_catalog"||!!e?.license_catalog_present,l=e?.license_enforced!==!1,d=n?.max_projects==null?"Unlimited":u(n?.max_projects),p=(s.status?.recent_errors||[]).find(k=>String(k?.source||"")==="middleware.db"&&/quarantin|corrupt/i.test(`${k?.message||""} ${k?.hint||""}`))||null,_=String(p?.hint||""),N=_.match(/Old DB moved to (.+?)\. Restore/i)?.[1]||_,H=p?`
  ...
  `}function Nt(e=null){let t=s.status?.endpoints||[],n=t.length>0,i=Array.isArray(s.lastReportData)&&s.lastReportData.length>0,r=a.reportsContent.dataset.reportPackReady==="1",o=s.reportsMessage,l=n?i?"Report is ready. Review risks, then export CSV.":"Step 1: Generate report. Step 2: Export CSV.":"";a.reportsFeedback.textContent=o?.text||l,a.reportsFeedback.className=`feedback feedback-banner${o?.kind==="error"?" danger":o?.kind==="success"?" success":o||l?" info":""}`,a.runReportButton.disabled=!n,a.runReportButton.title=n?"Step 1: Generate report":"No tracked APIs yet. Connect your first endpoint before generating a report pack.",a.exportReportCsv.disabled=!n,a.exportReportCsv.textContent=i?"2) Export CSV":"Generate then Export CSV",a.exportReportCsv.title=n?i?"Step 2: Download the latest generated report CSV.":"Generate report and export in one click.":"No tracked APIs yet. Connect your first endpoint before generating a report pack.";let d=a.reportEndpointSelect.value;if(a.reportEndpointSelect.innerHTML='<option value="">All tracked APIs</option>'+t.map(g=>`<option value="${g.endpoint_path}" ${g.endpoint_path===d?"selected":""}>${g.endpoint_path}</option>`).join(""),!e){if(!n){a.reportsContent.dataset.reportPackReady="0",a.reportsContent.innerHTML=`
      ...
      <div class="table-wrap">
        <table>
          ...
          <tbody>
            ${e.map(m=>`
              <tr>
                ${g.map(I=>`<td>${u(m[I])}</td>`).join("")}
              </tr>
            `).join("")}
          </tbody>
        </table>
      </div>
    `;return}let c=e.summary||{},p=e.digest||{},_=e.endpoint_report||e.endpointReport||null,A=c.health||{},N=c.summary||{},H=Array.isArray(c.active_anomalies)?c.active_anomalies:[],j=p.totals||{},k=_?.references||_?.baseline||{},P=Number(N.anomalies||0),v=Number(N.unconfirmed||0),C=P>=8?"high":P>0||v>0?"medium":"low",R=Number(k.coverage_pct??0),M=Number.isFinite(R)?Math.max(0,Math.min(100,R)):0,E=e.generated_at?ne(e.generated_at):ne(new Date().toISOString()),x=P>0?{label:"Needs attention",tone:"warning"}:v>0?{label:"Block release",tone:"danger"}:{label:"Safe for now",tone:"success"},y=x.tone==="success"?"resolved":x.tone==="danger"?"active":"acknowledged",T=P>0?"Needs attention: review Issues next and resolve high-priority changes before sharing this report.":v>0?"Block release: set up APIs next and finish setup for unconfirmed endpoints.":"Safe for now: monitoring is stable. Share this report and keep the current targets.",$=P>0?{view:"incidents",label:"Review Issues"}:v>0?{view:"api",label:"Set Up APIs"}:{view:"overview",label:"Open Overview"},V=g=>{let m=String(g||"medium").toLowerCase();return m==="critical"?"Critical":m==="high"?"High":m==="low"?"Low":"Medium"},f=H.length?H.slice(0,5).map(g=>`
      <div class="reports-issue-item reports-issue-item-${St(g)}">
        <div class="reports-issue-main">
          ...
          <div class="tiny" style="margin-top:4px;">
            ${S(String(g.kpi_field||"metric"))} moved ${Hs(g.pct_change)}
          </div>
          <div class="tiny muted" style="margin-top:4px;">
            expected ${u(g.expected_value??g.baseline_used)} \u2022 actual ${u(g.actual_value)}
          </div>
        </div>
        <span class="status-pill ${St(g)}">${S(V(g.severity))}</span>
      </div>
    `).join(""):P>0?'<div class="history-item">Open risks exist, but detailed rows are not loaded in this view. Review Issues to see the full queue.</div>':'<div class="history-item">No active issues right now.</div>';a.reportsContent.dataset.reportPackReady="1",a.reportsContent.innerHTML=`
    ...
    <div class="reports-health-banner reports-health-${C}">
      <div>
        ...
        <div class="tiny" style="margin-top:8px;">
          Status: ${u(A.status||"unknown")} \u2022 APIs: ${u(N.total_endpoints||0)} \u2022 Healthy: ${u(N.healthy||0)} \u2022 Issues: ${u(P)}
        </div>
        <div class="tiny muted" style="margin-top:4px;">
    ...
    <div class="report-grid reports-summary-grid">
      ...
      <div class="row-card reports-summary-card">
        ...
        <span>${u(j.runs||0)}</span>
        <div class="tiny muted" style="margin-top:6px;">Runs \u2022 Success ${u(j.success||0)} \u2022 Errors ${u(j.errors||0)}</div>
      </div>
    </div>

    <div class="row-card reports-next-step-card" style="margin-top:12px;">
      <strong>Recommended Next Step</strong>
      <div class="tiny" style="margin-top:8px;">${T}</div>
      <div class="toolbar" style="margin-top:10px;">
  ...
  `;let x=document.getElementById("view-guide-action");x&&r&&(x.onclick=()=>{Promise.resolve(r()).catch(y=>{Q(y,"Primary action failed.")})})}var Rr=new Set(["overview","playbook","api","incidents","errors","scheduler","settings","reports"]),Nr=new Set(["summary","incidents","uploads","configuration","history"]);function fn(e){let t=String(e||"").toLowerCase();return Rr.has(t)?t:String(s.defaultView||"api")}function Yn(e){let t=String(e||"").toLowerCase();return Nr.has(t)?t:"summary"}function Hr(){let e=new URLSearchParams(window.location.search),t=e.get("y_api");return{view:fn(e.get("y_view")),api:t&&t.trim()?t:null,tab:Yn(e.get("y_tab"))}}function ve(e="push"){if(e==="none")return;let t=new URL(window.location.href);t.searchParams.set("y_view",String(s.currentView||"overview")),s.selectedApi?t.searchParams.set("y_api",s.selectedApi):t.searchParams.delete("y_api"),s.currentView==="api"&&s.selectedApi?t.searchParams.set("y_tab",String(s.currentApiTab||"summary")):t.searchParams.delete("y_tab");let n=`${t.pathname}${t.search}${t.hash}`,i=`${window.location.pathname}${window.location.search}${window.location.hash}`;if(e==="push"&&n===i)return;let r={jin:!0,view:s.currentView,api:s.selectedApi,tab:s.currentApiTab};e==="replace"?window.history.replaceState(r,"",n):window.history.pushState(r,"",n)}async function fi(){let e=Hr(),t=new Set((s.status?.endpoints||[]).map(n=>n.endpoint_path));if(e.api&&t.has(e.api)?(s.selectedApi=e.api,s.apiWorkspaceOpen=!0):s.apiWorkspaceOpen=!1,s.currentView=fn(e.view),s.currentApiTab=Yn(e.tab),X(),s.currentView==="api"&&s.selectedApi){let n=await Ze(s.selectedApi);if(!n)return;s.activeApiDetail=n,de(n),(s.currentApiTab==="uploads"||s.currentApiTab==="history")&&await pn(s.selectedApi)}}async function Ze(e){try{return await mt(e)}catch(t){return Q(t,"Failed to load API details."),null}}async function et(e,t="push"){if(s.selectedApi=e,s.apiWorkspaceOpen=!0,s.selectedUploadAnalysisAt=null,s.currentView="api",s.currentApiTab="summary",s.uploadPage=1,s.runPage=1,ve(t),ln(),X(),cn(),ln(),s.currentView==="api"&&s.selectedApi){let n=await Ze(e);if(!n)return;s.activeApiDetail=n,de(n)}}async function Fr(e,t){e&&(e.preventDefault(),e.stopPropagation());let n=String(t||"").trim();if(n){s.selectedApi=n,s.apiWorkspaceOpen=!0,s.currentView="api",s.currentApiTab="summary",cn();try{await et(n,"replace")}finally{cn()}}}function Dr(){s.selectedApi=null,s.activeApiDetail=null,s.apiWorkspaceOpen=!1,s.currentApiTab="summary",X(),ln(),ve("replace")}async function Or(){let e=document.getElementById("license-key-input"),t=document.getElementById("license-feedback"),n=e?.value.trim(),r=String(s.status?.project?.license_backend||(s.status?.project?.license_catalog_present?"commercial_catalog":"legacy_demo"))==="commercial_catalog"||!!s.status?.project?.license_catalog_present,o=r?"business license":"legacy demo license";if(!n){w(`Please enter a ${o} key.`,"error");return}w(`Running ${o} activation...`,"success"),t&&(t.textContent="Contacting server...");try{let l=await fetch("/jin/api/v2/license/activate",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({key:n})}),d=await l.json();if(l.ok)w(`${r?"Business":"Legacy demo"} activation completed successfully!`,"success"),await Zt(),X();else{let c=d.detail||"Activation failed.";w(c,"error"),t&&(t.textContent=c,t.style.color="var(--danger-ink)")}}catch{w("Network error during activation.","error")}}function X(){let e={overview:["Overview","Start here to see project health and decide where to go next."],playbook:["PO Guide","PO flow for setup, target uploads, checks, issue review, and report packs."],api:["APIs",(s.selectedApi,"Pick one API, then follow Configure -> Uploads -> Checks.")],incidents:["Issues","See the current issues and take the next step."],errors:["Errors","Problems and next steps."],scheduler:["Watches","See scheduled checks and run them when needed."],settings:["Settings","Choose display, security, and saved-view behavior."],reports:["Reports","Generate a PO-ready report pack with health, risks, and next actions."]};document.querySelectorAll(".view").forEach(i=>i.classList.remove("active")),document.getElementById(`view-${s.currentView}`)?.classList.add("active"),document.querySelectorAll("#nav button").forEach(i=>{i.classList.toggle("active",i.dataset.view===s.currentView)}),a.pageTitle.textContent=e[s.currentView][0],a.pageSubtitle.textContent=e[s.currentView][1],a.topbar.style.display=["api","incidents","scheduler"].includes(s.currentView)?"none":"block";let t=s.status?.project,n=Fe(s.operatorHandle||localStorage.getItem("jin-operator-handle")||"default");s.operatorHandle=n,a.settingsSecurity.innerHTML=`
      ...
      `,Ir(),$e(),Fs(),Ds(),Ve(),yt(),Os(),Vs(),Le(),Ct(),Nt(),ln()}async function hi(e,t,n=0){let i={action:t};(t==="snoozed"||t==="suppressed")&&(i.snooze_minutes=n||60);let r=document.getElementById("drawer-note"),o=document.getElementById("drawer-owner"),l=document.getElementById("drawer-resolution-reason");if(s.selectedIncident&&Number(s.selectedIncident.id)===Number(e)){if(r&&r.value&&(i.note=r.value),o){let p=Dt(o.value);p&&(i.owner=p,o.value=p)}l&&l.value&&(i.resolution_reason=l.value)}i.owner||(i.owner=Dt(Dn()));let d=await fetch(`/jin/api/v2/anomaly/${e}/status`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(i)}),c=t==="acknowledged"?"marked in review":`updated to ${t}`;if(d.ok){let p=`Issue ${e} ${c}.`;w(p,"success"),fe(p,"success")}else{let p=`Failed to update issue ${e}.`;w(p,"error"),fe(p,"error")}await ce(!0)}async function Vr(e){let t=document.getElementById("drawer-note")?.value||"",n=document.getElementById("drawer-owner"),i=Dt(n?.value||"");n&&i&&(n.value=i);let r=document.getElementById("drawer-resolution-reason")?.value||"";(await fetch(`/jin/api/v2/anomaly/${e}/status`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:s.selectedIncident?.status||"active",note:t,owner:i||Dt(Dn()),resolution_reason:r})})).ok?(w("Issue notes saved.","success"),fe(`Notes saved for issue ${e}.`,"success")):(w("Failed to save issue notes.","error"),fe(`Could not save notes for issue ${e}.`,"error")),await ce(!0);let l=De().find(d=>Number(d.id)===Number(e));l&&Tt(l)}function Ur(e){let t=document.getElementById("drawer-resolution-reason");t&&(t.value=e)}async function yi(e,t){let n=await fetch(`/jin/api/v2/scheduler/${encodeURIComponent(e)}/${t}`,{method:"POST"});w(n.ok?`Scheduler action ${t} applied.`:`Failed scheduler action ${t}.`,n.ok?"success":"error"),await gt(),s.selectedApi?(s.detailCache.delete(s.selectedApi),await et(s.selectedApi)):X()}async function Wr(e,t){let n=await fetch(`/jin/api/v2/errors/${e}/status`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:t})});w(n.ok?`Error ${e} updated to ${t}.`:`Failed to update error ${e}.`,n.ok?"success":"error"),await Zt(),X()}async function zr(){if(!s.selectedApi)return;let e=Z();if(!e)return;let t=e.setup_config||e.config||{dimension_fields:[],kpi_fields:[]},n=Array.isArray(t.dimension_fields)?t.dimension_fields:[],i=Array.isArray(t.kpi_fields)?t.kpi_fields:[],r=String(t.time_field||"").trim(),o=dn(e,t),l=[];if(n.length||l.push("pick at least one Segment field"),i.length||l.push("pick at least one Metric field"),o&&!r&&l.push("pick one Time field"),l.length>0){let j=`Complete setup first: ${l.join(", ")}.`;a.configFeedback.textContent=j,w(j,"error");return}let d;if(a.configReferences.value.trim())try{d=JSON.parse(a.configReferences.value)}catch{a.configFeedback.textContent="Manual references must be valid JSON.",w("Manual references JSON is invalid.","error");return}let c=Number(a.configToleranceSimple.value||10);a.configNormal.value=String(c),a.configRelaxed.value=String(Math.round(c*2)),a.configStrict.value=String(Math.round(c/2));let p={rows_path:t.rows_path||null,time_field:r||null,time_end_field:t.time_end_field||null,time_profile:t.time_profile||"auto",time_extraction_rule:t.time_extraction_rule||"single",time_format:t.time_format||null};if(o||p.time_field){a.configFeedback.textContent="Checking time mapping using available samples...";try{let j=await fetch(`/jin/api/v2/config-mapping/test/${he(s.selectedApi)}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(p)}),k=await j.json();if(!j.ok||!k?.ok)throw new Error(k?.detail||k?.error||"Time mapping check failed.");let P=String(k?.sample_source||"none"),v=Number(k?.sample_count||0),C=Number(k?.summary?.success_count||0);if(v===0&&P==="none"){let R=String(s.selectedApi||"");R&&!s.mappingNoSamplesToastByApi?.[R]&&(s.mappingNoSamplesToastByApi=s.mappingNoSamplesToastByApi||{},s.mappingNoSamplesToastByApi[R]=!0,w("Time mapping preview could not run yet (no JSON samples found). Fix: hit this API once while Jin is running, or paste a JSON sample payload in Configuration > Time mapping > Test. If this API has no time field, you can ignore this.","warning"))}if(v>0&&C===0&&P!=="schema_example_rows"){let R="Time mapping could not parse sample rows yet. Setup can still be saved; mapping will validate after the first real check.";a.configFeedback.textContent=R,w(R,"warning")}v===0?a.configFeedback.textContent="No sample rows yet. Setup can still be saved; mapping will validate after first check.":v>0&&C===0?a.configFeedback.textContent="Time mapping could not parse schema example rows yet. Setup can still be saved; mapping will validate after the first real check.":a.configFeedback.textContent=`Time mapping check passed on ${C}/${v} sample row(s).`}catch(j){let k=`Could not validate time mapping: ${ue(j)}`;a.configFeedback.textContent=k,w(k,"error");return}}else a.configFeedback.textContent="No time field detected for this API shape. Setup will continue with Segment + Metric only.";let _={dimension_fields:n,kpi_fields:i,active_tolerance:a.configActiveTolerance.value,tolerance_relaxed:Number(a.configRelaxed.value||20),tolerance_normal:c,tolerance_strict:Number(a.configStrict.value||5),tolerance_pct:c,confirmed:!0,rows_path:p.rows_path,time_field:p.time_field,time_end_field:p.time_end_field,time_granularity:t.time_granularity||"minute",time_profile:p.time_profile,time_extraction_rule:p.time_extraction_rule,time_format:p.time_format,time_pin:!!t.time_pin,references:d},A=await fetch(`/jin/api/v2/config/${he(s.selectedApi)}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(_)}),N=await A.json();A.ok?(s.currentApiTab="uploads",ve("push"),w("Setup saved. Ready for target data.","success"),un(s.selectedApi)):(a.configFeedback.textContent=`Save failed: ${JSON.stringify(N)}`,w("Setup save failed.","error")),s.detailCache.delete(s.selectedApi),await ce(!0);let H=Z();H&&de(H)}window.saveConfig=zr;function qr(e){let t=e?.setup_config||e?.config||{},n=[],i=Array.isArray(t?.dimension_fields)?t.dimension_fields:[],r=Array.isArray(t?.kpi_fields)?t.kpi_fields:[],o=dn(e,t);return i.length||n.push("Segment"),r.length||n.push("Metric"),o&&!String(t?.time_field||"").trim()&&n.push("Time"),t?.confirmed===!1&&n.push("Save configuration"),n}function Qn(e){let t=String(s.selectedApi||"").trim();if(!t)return!1;let n=Z(),i=qr(n);if(!i.length)return!0;let r=`Before ${e}, complete setup in Configuration: ${i.join(", ")}.`;return a.uploadFeedback.textContent=r,w(r,"error"),Te(t,{title:"Insight: complete setup first",summary:r,kind:"error",actionType:"tab",actionValue:"configuration",actionLabel:"Open Setup"}),Ne("configuration","push"),!1}async function Jr(){if(!s.selectedApi){a.uploadFeedback.textContent="Choose an API first, then check your file.",w("Select an API first before checking a file.","error");return}if(!Qn("checking this file"))return;if(!a.uploadFile.files||!a.uploadFile.files.length){a.uploadFeedback.textContent="Choose a CSV or XLSX file first.";return}let e=new FormData;e.append("file",a.uploadFile.files[0]),a.previewUploadButton.disabled=!0,a.previewUploadButton.textContent="Checking\u2026",a.uploadFeedback.textContent="Checking file format and sample rows...";try{let t=await fetch(`/jin/api/v2/upload-preview/${he(s.selectedApi)}`,{method:"POST",body:e}),n=await t.text(),i={};if(n)try{i=JSON.parse(n)}catch{i={ok:t.ok,error:n}}else i={ok:t.ok};if((!i||typeof i!="object")&&(i={ok:t.ok,error:String(i||"")}),t.ok||(i.ok=!1,i.error||(i.error=`Server returned ${t.status} while checking the file.`)),a.uploadPreviewStep.style.display="",!i.ok)a.uploadPreviewBody.innerHTML=`
          ...
          `,a.uploadConfirmToolbar.style.display="none",a.uploadFeedback.textContent=i.error||"File check failed.";else{let r=Number(i.rows_in_file||i.rows_found||0),o=Number(i.columns_in_file||0),l=Number(i.file_size_bytes||0),d=!!i.is_large_upload,c="",p="",_=!0,A="",N="";try{let k=await fetch(`/jin/api/v2/config-mapping/test/${he(s.selectedApi)}`,{method:"POST",headers:{"Content-Type":"application/json"},body:"{}"}),P=await k.json(),v=Z(),C=v?.setup_config||v?.config||{},R=dn(v,C);if(k.ok&&P?.ok){let M=Number(P?.sample_count||0),E=Number(P?.summary?.success_count||0),x=String(P?.sample_source||"none");if(M===0&&x==="none"){let f=String(s.selectedApi||"");f&&!s.mappingNoSamplesToastByApi?.[f]&&(s.mappingNoSamplesToastByApi=s.mappingNoSamplesToastByApi||{},s.mappingNoSamplesToastByApi[f]=!0,w("Time mapping confidence is not validated yet because Jin has no runtime JSON samples for this API. Fix: hit the API once with Jin running, or paste a JSON sample payload in Configuration > Time mapping > Test.","info"))}let y=M>0?Math.round(E/M*100):0,T=M===0?"info":E===M?"success":E>0?"info":"danger",$=M===0?"Not validated yet":E===M?`Strong (${y}%)`:E>0?`Partial (${y}%)`:"Needs fix (0%)",V=Array.isArray(P?.summary?.warnings)?P.summary.warnings:[];c=`
            <div class="feedback ${T}" style="margin-top:10px;">
              <strong>Time mapping confidence: ${Be($)}</strong>
              <div class="tiny" style="margin-top:6px;">
                Parsed ${u(E)}/${u(M)} sample row(s) \u2022 source: ${Be(x)}.
              </div>
              ${V.length?`<div class="tiny muted" style="margin-top:6px;">${Be(String(V[0]))}</div>`:""}
            </div>
          `,p=M>0?` Time mapping parsed ${u(E)}/${u(M)} sample row(s).`:" Time mapping has no runtime sample yet.",R&&(M===0?(_=!1,A="Time mapping is not validated yet (no runtime JSON samples)."):E!==M&&(_=!1,A=`Time mapping needs a fix (${u(E)}/${u(M)} samples parsed).`))}else R&&(_=!1,A="Could not validate time mapping for this API.")}catch{let k=Z(),P=k?.setup_config||k?.config||{};dn(k,P)&&(_=!1,A="Could not validate time mapping for this API (network or server error).")}let H=String(s.selectedApi||"");if(H&&(s.uploadConfirmGateByApi=s.uploadConfirmGateByApi||{},s.uploadConfirmGateByApi[H]={ready:_,reason:A||void 0,hint:_?void 0:"Fix: hit the API once with Jin running, or paste a JSON sample in Configuration > Time mapping > Test, then re-run Check file."}),!_){let k=A||"Upload is blocked because setup is not ready yet.";N=`
          <div class="feedback danger" style="margin-top:10px;">
            <strong>Upload blocked</strong>
            <div class="tiny" style="margin-top:6px;">
              ${Be(k)} Fix this first in <strong>Configuration</strong>, then re-run <strong>Check file</strong>.
            </div>
          </div>
        `,w(`${k} Open Configuration to fix it.`,"error")}let j=(i.sample_rows||[]).length?`<table class="preview-table">
                ...
                <tbody>${i.sample_rows.map(k=>`
                  <tr>
                    <td>${k.group||"(all)"}</td>
                    ${i.metrics_detected.map(P=>`<td>${k.metrics[P]??"\u2014"}</td>`).join("")}
            ...
            <div class="upload-preview-ok">
              ...
              <div class="tiny muted" style="margin-top:8px;">
                File shape: ${u(r)} row(s) \xD7 ${u(o)} column(s) \u2022 ${Jn(l)}
              </div>
          ...
          `,a.uploadConfirmToolbar.style.display="",a.uploadButton.disabled=!_,a.uploadButton.title=_?"":A||"Upload blocked",a.uploadFeedback.textContent=_?`File check passed. ${u(r)} row(s) ready for upload.${p}`:`Upload blocked. ${A||""}`.trim()}}catch(t){a.uploadPreviewBody.innerHTML=`<div class="upload-preview-error">${Be(ue(t)||"Could not connect to server. Please try again.")}</div>`,a.uploadConfirmToolbar.style.display="none",a.uploadFeedback.textContent=ue(t),Q(t,"Could not check the file.")}finally{a.previewUploadButton.disabled=!1,a.previewUploadButton.textContent="Check file \u2192"}}var hn=()=>{a.runDetailDrawer.style.display="none"};a.runDetailClose.addEventListener("click",hn);a.runDetailDrawer.addEventListener("click",e=>{e.target===a.runDetailDrawer&&hn()});document.addEventListener("keydown",e=>{e.key==="Escape"&&a.runDetailDrawer.style.display!=="none"&&hn()});async function Gr(e){let t=document.getElementById("api-monitoring-progress");if(!t||!e)return;let n=Z(),i=v=>{if(!v)return{};if(typeof v=="string")try{let C=JSON.parse(v);return C&&typeof C=="object"?C:{}}catch{return{}}return typeof v=="object"?v:{}},r=v=>{let C=new Map,R=(M,E="")=>{if(M!=null){if(typeof M=="number"){E&&Number.isFinite(M)&&!C.has(E)&&C.set(E,M);return}if(typeof M=="string"){let x=M.trim(),y=Number(x);E&&x&&Number.isFinite(y)&&!C.has(E)&&C.set(E,y);return}if(Array.isArray(M)){M.forEach(x=>R(x,E?`${E}[]`:"data[]"));return}typeof M=="object"&&Object.entries(M).forEach(([x,y])=>{let T=E?`${E}.${x}`:x;R(y,T)})}};return R(v),Array.from(C.entries()).map(([M,E])=>({kpiField:M,value:E}))},o=v=>{if(!v||v.indexOf("|")===-1)return v;let[C,...R]=v.split("|"),M=R.map(E=>E.trim()).filter(Boolean).filter(E=>{let x=E.split("=")[0];return x!=="api_version"&&x!=="label"&&x!=="timestamp"&&x!=="_jin_id"}).sort();return M.length?`${C}|${M.join("|")}`:C},l=v=>{let C=String(v||"").trim().toLowerCase();return C?C.replace(/\[\]/g,"").replace(/^data\./,"").replace(/^payload\./,""):""},d=new Map;(n?.references||[]).forEach(v=>{let C=String(v?.grain_key||""),R=String(v?.kpi_field||"");if(!C||!R)return;let M=v?.expected_value,E=v?.tolerance_pct,x=M==null?null:Number(M),y=E==null?null:Number(E),T={expected:Number.isFinite(x)?x:null,tolerance:Number.isFinite(y)?y:null},$=l(R);d.set(`${C}__${R}`,T),$&&$!==R&&d.set(`${C}__${$}`,T);let V=o(C);V&&(d.has(`${V}__${R}`)||d.set(`${V}__${R}`,T),$&&!d.has(`${V}__${$}`)&&d.set(`${V}__${$}`,T))});let c=Number(n?.config?.tolerance_normal??n?.config?.tolerance_pct??10),p=Array.isArray(e.comparisons)?e.comparisons.filter(v=>v?.kpi_field):[],_=i(e.kpi_json),A=r(_),H=(p.length?p.map(v=>{let C=v.actual_value??v.actual,R=String(e.grain_key||""),M=o(R),E=String(v.kpi_field||""),x=l(E),y=d.get(`${R}__${E}`)||(x?d.get(`${R}__${x}`):void 0)||d.get(`${M}__${E}`)||(x?d.get(`${M}__${x}`):void 0),T=v.expected_value??v.expected??y?.expected??null,$=v.allowed_tolerance_pct??y?.tolerance??c,V=v.pct_change??(T==null||C==null||Number(T)===0?null:(Number(C)-Number(T))/Math.abs(Number(T))*100),f=String(v.status||""),g=f&&!(f==="missing_reference"&&T!=null)?f:T==null?"missing_reference":V!=null&&Math.abs(Number(V))>Number($)?"mismatch":"match";return{...v,kpi_field:v.kpi_field,actualValue:C,expectedValue:T,pctChange:V,status:g,message:v.message||(g==="match"?`${v.kpi_field} matched the expected target.`:g==="missing_reference"?`${v.kpi_field} has no uploaded targets for this grain.`:`${v.kpi_field} did not match the uploaded targets.`)}}):A.map(({kpiField:v,value:C})=>{let R=Number(C),M=String(e.grain_key||""),E=l(v),x=o(M),y=d.get(`${M}__${v}`)||(E?d.get(`${M}__${E}`):void 0)||d.get(`${x}__${v}`)||(E?d.get(`${x}__${E}`):void 0),T=y?.expected??null,$=y?.tolerance??c,V=T==null||T===0?null:(R-T)/Math.abs(T)*100,f=T==null?"missing_reference":V!=null&&Math.abs(V)>$?"mismatch":"match";return{kpi_field:v,actualValue:R,expectedValue:T,pctChange:V,status:f,message:T==null?`No uploaded targets found for ${v} on this grain.`:`Compared using uploaded targets (allowed +/-${$.toFixed(1)}%).`}})).filter(v=>v&&String(v.kpi_field||"").trim().length>0);if(!H.length){let v=Object.keys(_||{}),C=v.length?`Captured keys: ${v.slice(0,6).join(", ")}${v.length>6?", ...":""}.`:"No KPI fields were returned by this run.";t.dataset.endpoint=s.selectedApi||"",t.dataset.source="live-check",t.innerHTML=`<div class="empty">No comparable KPI values were captured for this run. ${C}</div>`,t.style.display="block";return}let j=H.filter(v=>v.status!=="match").length,k=j===0?{title:"Data Quality: Excellent",sub:"All metrics are within your defined targets.",class:"success"}:{title:`Action Required: ${j} Issue${j>1?"s":""} Found`,sub:"Some metrics have drifted beyond acceptable limits.",class:"danger"},P=`
    <div class="results-auto-show">
      ...
      <div class="table-wrap">
        <table class="run-detail-table" style="width:100%; border-collapse:collapse;">
          ...
          <tbody>
            ${H.map(v=>{let C=v.pctChange,R=v.status!=="match",M=R?"var(--anomaly)":"var(--healthy)";return`
                <tr style="border-bottom:1px solid var(--line);">
                  ...
                  <td style="padding:12px; font-family:var(--font-mono); font-size:13px; color:${M}; font-weight:600;">
  ...
  `;t.dataset.endpoint=s.selectedApi||"",t.dataset.source="live-check",t.innerHTML=P,t.style.display="block"}async function Kr(e="summary"){let t=s.selectedApi;if(!t||!Qn("running a manual check"))return;a.checkNowButton.disabled=!0,a.checkNowButton.textContent="Checking...",Ne(e,"none");let i=document.getElementById("api-monitoring-progress");i&&(i.dataset.endpoint=t,i.dataset.source="live-check-loading",i.innerHTML=`
      <div class="row-card">
    ...
    `,i.style.display="block");try{let r=await fetch(`/jin/api/v2/check/${he(t)}`,{method:"POST"}),o=await r.json();if(r.ok){await new Promise(c=>setTimeout(c,300)),s.detailCache.delete(s.selectedApi||""),s.detailCache.delete(t);let l=await mt(s.selectedApi||"");if(Mr(t,l),l.history&&l.history.length>0)await Gr(l.history[0]);else{let c=document.getElementById("api-monitoring-progress");c&&(c.innerHTML=`
