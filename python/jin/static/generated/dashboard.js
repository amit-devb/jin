...
<body style="margin:0;background:#f4efe7;color:#1f1912;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
          ...
          ${F.length?`
                ...
                ${ue.length?`
                  <details class="upload-analysis-inline-more">
                    <summary>Show ${u(ue.length)} additional matched KPI(s)</summary>
  ...
  `}function pr(){return(s.status?.recent_errors||[]).filter(t=>{let n=t.category?t.category[0].toUpperCase()+t.category.slice(1):Bs(t.source),i=t.severity||Ln(t.source),r=sn(t),o=`${t.source} ${t.message} ${t.hint||""} ${t.endpoint_path||""}`.toLowerCase(),l=!s.errorSearch||o.includes(s.errorSearch.toLowerCase()),d=!s.errorStatusFilter||r===s.errorStatusFilter,c=!s.errorCategoryFilter||n===s.errorCategoryFilter,p=!s.errorSeverityFilter||i===s.errorSeverityFilter;return l&&d&&c&&p})}function _e(){let e=s.status?.project,t=Me(),n=e?.trust_score??100,i=String(e?.tier||"free").toLowerCase(),r=e?.license_enforced!==!1,o=r?e?.project_limit==null?i==="free"?"1":"unlimited":String(e.project_limit):"unlimited",l=t?`
  ...
  `,j=E?`
    ...
    `:"",le=new Map,Y=(w,M,U)=>{let q=String(w||"").trim();if(!q)return;let Z=le.get(q)||[];Z.some(W=>W.role===U)||(Z.push({label:M,role:U}),le.set(q,Z))};Y(I?.segmentCandidates?.[0],"Best Segment","segment"),Y(I?.metricCandidates?.[0],"Best Metric","metric"),Y(I?.timeCandidates?.[0],"Best Time","time");let ue=P.map(w=>{let{name:M,shownName:U,hasTechnicalPath:q,type:Z,role:W}=w,te=t.time_extraction_rule||"single",$e=le.get(M)||[],ve=$e.length?` best-candidate ${$e.map(ie=>`best-${ie.role}`).join(" ")}`:"",G=String(W==="time"?s.timePreview||`Model-selected time field: ${U||M}`:s.timePreview||"No timeline preview yet"),ae=/no sample run yet|no recent sample yet|choose a time field|choose your business time field|no sample value found/i.test(G);return`
      <div class="field-role-card ${W==="time"?"active-time":W==="dimension"?"active-dimension":W==="kpi"?"active-kpi":W==="exclude"?"active-exclude":""}${ve}" data-field-name="${M}">
        <div class="field-info">
          <div class="field-name">
            ${S(U||M)}
            ${$e.map(ie=>`<span class="chip field-candidate-chip field-candidate-chip--${ie.role}">${ie.label}</span>`).join("")}
  ...
  `;let K=T.length?`for every unique <strong>${T.join(", ")}</strong>`:"across your entire dataset",se=B.length?`monitor <strong>${B.join(" and ")}</strong>`:"track any numbers yet",Se=document.getElementById("config-step-3-container");if(Se){let w=B.length>0||T.length>0||l,M=String(s.selectedApi||""),U=s.autoSuggestSummaryByApi||{},q=M?U[M]:null,Z=Ls(s.activeApiDetail),W=i.response_model_present===!1,te=W?null:ot(i.fields||i.schema_contract?.fields||[]),$e=!!te?.ready,ve=!!(te&&te.candidateCount>0),G=W?"Define a Pydantic response model first. Jin needs it to discover fields and time candidates before setup can be saved.":q?`${q.headline} ${q.details}`:$e?"Jin can pre-fill Segment, Metric, and Time from the Pydantic response model even before traffic arrives.":Z?`Use auto-suggest to pre-fill setup from recent API traffic or Pydantic examples. ${te?.detail||""}`:`${te?.summary||"The response model is present, but Jin needs clearer typed fields or examples to pre-fill setup."} ${te?.detail||""}`.trim();Se.innerHTML=`
    ...
    `}else a.apiCoreInsight.style.display="none",a.apiCoreInsight.innerHTML="";let z=e.current_kpis||[];a.apiKpis.innerHTML=z.length?z.map(w=>`
      ...
      `}).join(""):ke("No issues for this API yet."),o.length){let w=ct(o,s.runPage,8);s.runPage=w.page;let M=o.filter(W=>String(W?.trigger||"").toLowerCase()==="manual").length,U=o.filter(W=>String(W?.trigger||"").toLowerCase()==="scheduler").length,q=Number(T?.mismatch_runs||0),Z=o.filter(W=>{let te=String(W?.status||"").toLowerCase();return te==="error"||te==="failed"}).length;a.apiRunTable.innerHTML=`
        ...
        <div class="table-wrap"><table class="row-table">
          ...
          <tbody>
            ${w.items.map(W=>{let te=Number(W?.anomalies_detected||0),$e=Math.max(te,q),ve=or(String(W?.status||"unknown"),$e),G=Number(W?.grains_processed||0);return`
                <tr>
                  <td>
                  ...
                  <td>
                    <span class="status-pill ${ve.pillClass}" title="${S(ve.tooltip)}">${S(ve.label)}</span>
                    <div class="tiny muted" style="margin-top:6px;">Duration: ${S(rr(W?.duration_ms))}</div>
                  </td>
                  <td>
                    <div>${u(G)} segment(s)</div>
                    <div class="tiny ${$e>0?"":"muted"}">${u($e)} mismatch(es)</div>
                  </td>
                  <td>
      ...
      `}else{let w=(e.history&&e.history.length?e.history:e.recent_history)||[],M=G=>{if((Array.isArray(G?.comparisons)?G.comparisons.filter(pe=>pe?.kpi_field):[]).length)return!0;let ie={};if(G?.kpi_json&&typeof G.kpi_json=="string")try{let pe=JSON.parse(G.kpi_json);pe&&typeof pe=="object"&&(ie=pe)}catch{ie={}}else G?.kpi_json&&typeof G.kpi_json=="object"&&(ie=G.kpi_json);let je=pe=>{if(pe==null)return!1;if(typeof pe=="number")return Number.isFinite(pe);if(typeof pe=="string"){let Ge=pe.trim();return Ge.length>0&&Number.isFinite(Number(Ge))}return Array.isArray(pe)?pe.some(Ge=>je(Ge)):typeof pe=="object"?Object.values(pe).some(Ge=>je(Ge)):!1};return je(ie)},U=new Map;w.forEach(G=>{let ae=G.observed_at;ae&&(U.has(ae)||U.set(ae,[]),U.get(ae).push(G))});let q=new Set,Z=new Set;(e.references||[]).forEach(G=>{let ae=String(G?.grain_key||"").trim();if(!ae)return;let ie=Ke(ae),je=String(G?.kpi_field||"").trim(),pe=Rt(je);q.add(ae),q.add(ie),je&&Z.add(`${ie}__${je}`),pe&&Z.add(`${ie}__${pe}`)});let W=G=>{let ae=Array.isArray(G?.comparisons)?G.comparisons.filter(pe=>pe?.kpi_field):[];if(ae.length){let pe=Ke(String(G?.grain_key||"").trim());return ae.every(Ge=>{let Zn=String(Ge?.kpi_field||"").trim(),ji=Rt(Zn);return!Z.has(`${pe}__${Zn}`)&&!Z.has(`${pe}__${ji}`)})}if(!M(G))return!1;let ie=String(G?.grain_key||"").trim();if(!ie)return!0;let je=Ke(ie);return!q.has(ie)&&!q.has(je)},te=Array.from(U.entries()).map(([G,ae])=>({observed_at:G,obs_count:ae.length,status:ae.some(ie=>(e.anomaly_history||[]).some(je=>je.detected_at===G&&Ke(String(je.grain_key||""))===Ke(String(ie.grain_key||""))))?"anomaly":ae.some(ie=>M(ie))?ae.every(ie=>W(ie))?"no_baseline":"healthy":"no_data"})).sort((G,ae)=>ae.observed_at.localeCompare(G.observed_at)),$e=G=>G==="anomaly"?{pillClass:"danger",label:"Needs attention",tooltip:"At least one metric moved outside your allowed tolerance."}:G==="no_data"?{pillClass:"warning",label:"No comparable values",tooltip:"A run happened, but no comparable numeric values were captured."}:G==="no_baseline"?{pillClass:"acknowledged",label:"Needs baseline",tooltip:"Values were captured, but this segment does not have an uploaded baseline yet."}:{pillClass:"healthy",label:"Within target",tooltip:"All compared metrics stayed within tolerance."},ve=ct(te,s.runPage,8);s.runPage=ve.page,a.apiRunTable.innerHTML=te.length?`
          <div class="table-toolbar">
          ...
          <div class="table-wrap"><table class="sticky-first">
            ...
            <tbody>
              ${ve.items.map(G=>{let ae=$e(G.status);return`
                <tr>
          ...
          ${dt("runs",ve.page,ve.totalPages)}
        `:ke("No checks for this API yet.")}s.uploadSort==="kpi_asc"&&(s.uploadSort="uploaded_at_desc");let Y=pt(e.upload_activity||[],s.uploadSort,"uploaded_at"),ue=new Map;Y.forEach(w=>{let M=String(w?.uploaded_at||""),U=String(w?.grain_key||""),q=`${M}__${U}`;ue.has(q)||ue.set(q,{uploaded_at:w?.uploaded_at||null,grain_key:w?.grain_key||null,upload_source:w?.upload_source||null,metrics:[]}),ue.get(q).metrics.push({kpi_field:String(w?.kpi_field||""),expected_value:w?.expected_value==null?null:Number(w?.expected_value)})});let Ae=Array.from(ue.values()).map(w=>({...w,metrics:[...w.metrics].sort((M,U)=>String(M.kpi_field||"").localeCompare(String(U.kpi_field||"")))})),K=ct(Ae,s.uploadPage,6);s.uploadPage=K.page;let se=[...e.upload_analysis_history||[]].sort((w,M)=>String(M?.analyzed_at||"").localeCompare(String(w?.analyzed_at||""))),Se=new Map;e.metrics&&e.metrics.forEach(w=>{w.calculation&&w.calculation.field&&Se.set(w.calculation.field,w.name)});let qe=se.length?`
                  ...
                  <div class="upload-event-metrics">
                    ${w.metrics.slice(0,2).map(M=>`
                      <div class="upload-event-metric">
                        <strong>${S(Se.get(M.kpi_field)||ft(M.kpi_field))}</strong>
